...
const PIE_COLORS = ["#3b82f6", "#f59e0b", "#10b981", "#ef4444", "#8b5cf6", "#ec4899", "#f97316", "#06b6d4", "#84cc16", "#6366f1"]
...
export function TopSitesCard({ sites, maxBookings, periodLabel }: TopSitesCardProps) {
  const [view, setView] = useState<"list" | "chart">("list")

  const pieData = sites.map((site) => ({
    name: site.siteName,
    value: site.bookingsCount,
  }))
  ...
  return (
    <div className="rounded-[20px] bg-card p-5 h-full">
      <div className="flex items-start justify-between mb-1">
        <h2 className="font-header text-lg uppercase tracking-wide">Top Sites</h2>
      ...
      <p className="text-sm text-muted-foreground mb-4">Les plus réservés — {periodLabel || "ce mois"}</p>

      {sites.length > 0 ? (
        view === "list" ? (
          <div className="space-y-3">
            {sites.map((site, index) => {
              const percentage = maxBookings > 0 ? (site.bookingsCount / maxBookings) * 100 : 0
